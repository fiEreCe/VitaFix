...
test('PF-001 through PF-004 closes a verified evidence-driven loop', async () => {
  const store = new Map(); const repository = { async create(v) { store.set(v.id, structuredClone(v)); return store.get(v.id); }, async get(id) { return store.get(id); }, async save(v) { store.set(v.id, structuredClone(v)); return store.get(v.id); }, async claimAnalysis(id, claim) { const value = store.get(id); if (!value || !claim.fromStates.includes(value.state)) return null; const from = value.state; value.state = claim.to; value.transitions.push({ from, to: claim.to, event: claim.event, toolName: claim.toolName, at: claim.at }); return value; } };
