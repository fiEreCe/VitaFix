...
![PF-001 证据驱动的简历优化 Agent 图形化流程图](../assets/pf001-resume-agent-flow.png)

流程图采用从上到下的单一主干，动态追问、审核修正、用户编辑和停止条件位于左右旁支。默认使用 PNG 展示，兼容 Markdown 预览和常见文档工具。[SVG 矢量源文件](../assets/pf001-resume-agent-flow.svg)可直接导入 Figma 继续编辑。下方 Mermaid 内容作为流程逻辑源码保留。
...
| 10 | 进入重新评估 | 将原文和最终文本交给 PF-003 | 开始修改—重评 |
...
PF-003 负责评估修改带来的变化，不得把“用户选择继续”解释为“事实已经验证”。
