  ...
  <div class="agent-workbench">
    ...
    <main v-else class="content">
      ...
      <template v-if="task">
        ...
        <section v-if="latestValidation(task)" class="card"><h3>本轮修改效果验证</h3><p>{{ outcomeLabel(latestValidation(task).changeOutcome) }} · {{ safetyLabel(latestValidation(task).safetyStatus) }}</p><p class="candidate">{{ latestValidation(task).diff.before }} → {{ latestValidation(task).diff.after }}</p><p v-for="issue in latestValidation(task).remainingIssues" :key="issue.type" class="risk">{{ issue.type }}</p><van-button v-if="latestValidation(task).safetyStatus === 'blocked' && task.state !== 'completed_with_risk'" type="warning" block round @click="completeWithRisk">确认保留风险内容</van-button></section>
